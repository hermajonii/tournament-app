  ...
  const sortedPlayers = Object.entries(players).sort(
    ([aName, aScore], [bName, bScore]) => {
      if (sortConfig.key === "name") {
        ...
        return sortConfig.dir === "asc" ? aScore - bScore : bScore - aScore;
      }
    }
  );
    ...
    <div className="bg-black pb-2">
      ...
      {activeTab === "teams" &&
        teams.map((team) => (
          <div key={team.team} className="my-5 px-3 mx-3">
            <h5 className="pt-3 bg-gradient text-light rounded-top p-2 m-0 border-dark">
              {t("team")} {team.team} - {t("members")}
            </h5>
            <table className="table table-bordered border-0">
              <thead>
                <tr>
                  <td className=" bg-black text-orange text-center pb-1 align-middle border-dark form-subtitle">
                    {" "}
                    {t("number")}
                  </td>
                  <td className="bg-black text-orange text-center pb-1 align-middle border-dark form-subtitle">
              ...
              <tbody>
                {Array.from({ length: numOfPlayers }).map((_, i) => (
                  <tr key={i}>
                    <td className="col-1 bg-black text-light text-center pb-1 align-middle border-dark">
                      {" "}
                      {i + 1}.
                    </td>
      ...
      {activeTab === "results" && wins && (
        <div className="m-5 mb-0 p-3">
          <h3 className="pt-3 bg-gradient text-light rounded-top p-2 m-0 border-dark">
            {t("numOfWins")}:{" "}
          </h3>
          <table className="table table-bordered table-stripped border-0">
            <thead>
              <tr>
                <td className="col-6 bg-black text-orange text-center pb-1 align-middle border-dark form-subtitle">
                  {" "}
                  {t("team")}
                </td>
                <td className="bg-black text-orange text-center pb-1 align-middle border-dark form-subtitle">
            ...
            <tbody>
              {wins.map((team) => (
                <tr key={team.team}>
                  <td className="col-6 bg-black text-light text-center pb-1 align-middle border-dark">
                    {" "}
                    {t("team")} {team.team}{" "}
                  </td>
                  <td className="bg-black text-light text-center pb-1 align-middle border-dark">
      ...
      {activeTab === "results" &&
        ...
        Object.keys(courts).map((courtNum) => (
          <div key={courtNum} className="mx-5 mb-5 p-3">
            ...
            <h3 className="pt-3 bg-gradient text-light rounded-top p-2 m-0 border-dark">
              {t("field")} {parseInt(courtNum) + 1}
            </h3>
            <table className="table table-bordered text-center table-striped mt-0 border-0">
              <thead>
                <tr>
                  <th className="bg-black text-orange col-6 border-dark form-subtitle">
                    {t("game")}
                  </th>
                  <th className="bg-black text-orange col-6 border-dark form-subtitle">
                    {t("score")}
              ...
              <tbody>
                {courts[courtNum].map((g, idx) => (
                  <tr key={idx}>
                    <td className="bg-black text-light border-dark">
                      {g.game}
                    </td>
                    <td className="bg-black text-light border-dark">
                      {g.result || "–"}
      ...
      {activeTab === "players" && Object.keys(players).length > 0 && (
        <div className="m-5">
          <h3 className="pt-3 bg-gradient text-light rounded-top p-2 m-0 border-dark">
            {t("numOfWins")}:{" "}
          </h3>
          <table className="table table-bordered border-0">
            <thead>
              <tr>
                ...
                <td
                  className="bg-black text-orange text-center pb-1 align-middle border-dark form-subtitle"
